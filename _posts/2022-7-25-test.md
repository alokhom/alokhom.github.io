...
- We will need to define volumes that refer to a configMap and google-storage-s3-json secret. 
- The jolokia-share volume helps provide the jar from the initContainer to the medusa container.
```
 volumes:
   - name: jolokia-share
     emptyDir: {}
...
```
- Add mounts on the cassandra container.
```
     volumeMounts:
       - name: medusa-scripts
