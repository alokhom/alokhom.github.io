...
Using medusa as a backup tool on the vanilla sts cassandra cluster - has no downtime.
--------------------------------------------------------------------------------------
...
```
Modifying the vanilla STS yaml file.
---------------------------------------
...
```
   - name: medusa
     ...
     securityContext: {}
```
Apply the configMap to the namespace if not default namespace as cassandra.ConfigMap.yaml.It offers 
...
key takeaways of the configMap. We have the medusa.ini, 